<?php
	...
	if(isset($_POST["submit"])){
		...
		if(empty($_POST["name_login"])){
			$err1 = "Tên không được để trống.";
			$x=2;
		}
		if(empty($_POST["email_login"])){
			$err2 = "email không được để trống.";
			$x=2;
		}
		if(empty($_POST["pass_login"])){
			$err3 = "Pass không được để trống.";
			$x=2;
		}
            ...
            //    ///////////////////
              ...
              // exit;
                 ...
                 if($x==1){
                    ...
                    move_uploaded_file($_FILES['avatar']['tmp_name'], './img_text/'.$_FILES['avatar']['name']);
                    echo 'File Uploaded';
                }
            }
        }
        else{
			$err5 = 'Bạn chưa chọn file upload.';
			$x = 2;
        }


		if($x==1){
			// Nếu tất cả các trường đều có giá trị, xử lý tiếp
	
			$avatar = $_FILES['avatar']['name'];
			$sql = "INSERT INTO user 
							(email, pass, name, avatar)
					 VALUES (
					'".$_POST['email_login']."', 
                	'".md5($_POST['pass_login'])."', 
                	'".$_POST['name_login']."', 
                	'".$_FILES['avatar']['name']."')";

					if ($result = $con->query($sql)) {
						$err6= "thêm thanh công";
					}else{
						$err6= "thêm thanh that bại";
					}
				...
				<div class="col-sm-4">
					<div class="signup-form"><!--sign up form-->
						...
						<form action="#" method="POST" enctype="multipart/form-data">
							<input type="text" name="name_login" placeholder="Name"/>
							<p><?php echo $err1; ?></p>
